/* 
 * Program simulates the UNIX wc commmand with particularly option -lwc
 */
...
// Print numbers of lines, words, and characters from stdin or 1+ files
void print_lwc_multi( int start, int argc, char* argv[], int* v, int all ) {
    FILE* fp;
    int k = start, total_chars = 0, total_words = 0, total_lines = 0;
    for ( ; k < argc; k++ ) {
        fp = fopen( argv[k], "r" );
        if ( !fp )
            ...
            total_chars += fc.chars; total_words += fc.words; total_lines += fc.lines;
            fclose( fp );
        }
    }
    if ( k > start + 1 ) {
...
int main( int argc, char* argv[] ) {
    ...
    if ( v[0] == -1 ) print_lwc_multi( 2, argc, argv, v, 0 );
    // Second argument does not have option, read and count files from second argument
    else if ( v[0] == -2 ) print_lwc_multi( 1, argc, argv, v, 1 );
